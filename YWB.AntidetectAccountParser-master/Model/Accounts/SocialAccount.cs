...
{ 
    ...
    public class SocialAccount 
    {
        ...
        public List<LoginPassword> LoginsPasswords { get; set; } = new List<LoginPassword>();
        
        public string AccountInfoString { get; set; }
        public string DisplayInfoString { get; set; }
        ...
        public List<string> Passwords { 
            get {
                if (_passwords.Count == 0)
                    return new List<string>() { "Нет паролей" };
                return _passwords;
            }
        ...
        public void SavePasswords()
        {
            try
            {
                ...
                foreach (var item in Passwords)
                    sb.AppendLine(item);
                File.WriteAllText(System.IO.Path.Combine(Path, "_pswrds.txt"), sb.ToString());

            } catch (Exception ex)
            {
                throw new Exception($"Не удалось сохранить пароли для аккаунта {AccountName}");
            }
        }
